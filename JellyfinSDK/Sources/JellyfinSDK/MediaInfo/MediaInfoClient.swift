...
public protocol MediaInfoClient: Sendable {
    func playbackInfo(request: PlaybackInfoRequest) async throws -> PlaybackInfoResponse
}

public actor DefaultMediaInfoClient: MediaInfoClient {
    ...
    public func playbackInfo(request: PlaybackInfoRequest) async throws -> PlaybackInfoResponse {
        let url = settings.serverAddress.appending(path: "Items/\(request.itemId)/PlaybackInfo")
